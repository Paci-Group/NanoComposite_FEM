...
import onelab
import sys
sys.path.append('path/to/current/dir')
import numpy as np
...
summary = results + '/sphere_size_series_eps_inf.csv'
...
try:
    ...
    eps_incs = np.array([210.])
    eps_mat = 3.04 # 9.26

    Ss = np.loadtxt('sidelengths.csv', delimiter=',', dtype=float)
    eps_ints = np.array([eps_mat for _ in Ss])
    t = 1
    lcoarse=0.5
    per2pi = 50
    ...
    R = 3.265667465  # radius of inclusion
    for S in Ss:
        # remesh only when t or vl changes
        generate_mesh(t, [R]*3, S, per2pi=per2pi, lcoarse=lcoarse)
        for eps_inc, eps_int in zip(eps_incs, eps_ints):
            vl = 4/3*np.pi*R**3 / np.prod(S)
            ...
            _, remesh = get_eps_effective(eps_inc, eps_int, eps_mat, t, [R]*3, S, target, per2pi=per2pi, lcoarse=lcoarse)
