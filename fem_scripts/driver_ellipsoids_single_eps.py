...
composite = client.defineString('Composite', value='ellipsoid')
...
summary = results + '/eps_infty_rods_discs_210_fine.csv'

def generate_mesh(t, R, S, per2pi=20, lcoarse=1.5):
...
def get_eps_effective(eps_inc, eps_int, eps_mat, t, R, S, target=1E-15, per2pi=20, lcoarse=1.5):
    ...
    with open(composite_pro, 'r') as f:
        for line in f.readlines():
            ...
            # For spheres without interface
            # if line.startswith("  epsr[Region[5]] ="):
            #     line = f"  epsr[Region[5]] = {eps_inc};\n"
            # elif line.startswith("  epsr[Region[4]] ="):
            #     line = f"  epsr[Region[4]] = {eps_mat};\n"
            lines.append(line)
    ...
    # volume loading
    vl = 4/3 * np.pi * Rx * Ry * Rz / (Sx * Sy * Sz)

    client.sendInfo(f'Beginning Calculation for VL~{vl}% -- Side Lengths: {Sx} {Sy} {Sz}, Rs: {Rx} {Ry} {Rz}, Interface: {t}')
    ...
    while True:   # calculate solution until energy criterion met
        ...
        if max_e < target or nit > 10:
            with open(summary, 'a') as f:
                f.write(f"{Sx},{Sy},{Sz},{Rx},{Ry},{Rz},{vl},{vl_actual},{eps_inc},{eps_int},{eps_mat},{t},{lcoarse},{per2pi},{len(tets)},{target},{mean_e},{std_e},{max_e},{min_e},{energy},{epsr},{V},{di},{dint},{dh},{dtot}\n")
        ...
        generate_mesh(t, R, S, per2pi=per2pi, lcoarse=lcoarse)
...
try:
    with open(summary, 'w+') as f:
        f.write("Sx,Sy,Sz,Rx,Ry,Rz,vl,vl_fem,eps_inc,eps_int,eps_mat,t_interface,lcoarse,per2pi,Nel,target E,mean E,Std E,max E,min E,Energy,Eps,V,Di,Dint,Dh,Dtot\n")
    ...
    Ss = np.array([[12.9872736, 12.987172, 17.1742575],
                   [12.9872736, 17.1742575, 12.987172],
                   [17.19457789, 17.19457789, 13.14401583],
                   [17.19457789, 13.14401583, 17.19457789]])

    Rs = np.array([[6.687818133, 6.907695124, 9.047488074],
                   [6.687818133, 9.047488074, 6.907695124],
                   [9.307552669, 9.349788508, 7.085479126],
                   [9.307552669, 7.085479126, 9.349788508]]) / 2
    ...
    for i, S in enumerate(Ss):
        R = Rs[i]
        vl = 4/3 * np.pi * np.prod(R) / np.prod(S)
        vol = 4/3 * np.pi * np.prod(R)
        # remesh only when t or vl changes
        generate_mesh(t, R, S, per2pi=per2pi, lcoarse=lcoarse)
        # client.sendInfo(f'eps_inc: {eps_inc}, eps_int: {eps_int}, eps_mat: {eps_mat}, t_int: {t}')
        client.sendInfo(f'per2pi: {per2pi}, vl: {vl}, eps_inc: {eps_inc}, eps_int: {eps_int}, eps_mat: {eps_mat}, t_int: {t}')
        _, remesh = get_eps_effective(eps_inc, eps_int, eps_mat, t, R, S, target, per2pi=per2pi, lcoarse=lcoarse)
