...
summary = results + '/eps_infty_cpmd_fine_2.csv'
...
try:
    ...
    eps_incs = np.arange(150, 251, 1)
    eps_ints = np.array([9.2])
    eps_mat = 9.2
    Ss = np.array([8.9302, 12.926, 17.077])
    ts = np.array([1.])
    vls = np.array([21.39098594, 6.754800892, 2.905233845])
    cpmd_values = np.array([11.94])
    lcoarse=1.5
    per2pi = 20

    client.sendInfo(f'Beginning Calculations...')
    for i, vl in enumerate(vls):
        vol = (vl / 100) * Ss[i]**3  # 0.067548081 * 12.926**3    # 145.8832635203091
        R = (3 / 4 / np.pi * vol)**(1/3)  # get sidelength of inclusion
        for j, t in enumerate(ts):
            # remesh only when t or vl changes
            generate_mesh(t, [R]*3, [Ss[i]]*3, per2pi=per2pi, lcoarse=lcoarse)
            for eps_inc in eps_incs:
                for eps_int in eps_ints:
                    ...
                    _, remesh = get_eps_effective(eps_inc, eps_int, eps_mat, t, [R]*3, [Ss[i]]*3, target, per2pi=per2pi, lcoarse=lcoarse)
