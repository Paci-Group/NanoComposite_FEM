    ...
    while True:   # calculate solution until energy criterion met
        ...
        if max_e < target or nit > 10:
            with open(summary, 'a') as f:
                f.write(f"{Sx},{Sy},{Sz},{rf},{Rx},{Ry},{Rz},{vl},{vl_actual},{eps_inc},{eps_int},{eps_mat},{t},{0.5},{50},{len(tets)},{target},{mean_e},{std_e},{max_e},{min_e},{energy},{epsr},{V},{di},{dint},{dh},{dtot}\n")
...
try:
    ...
    Rs = np.array([[5.717741934, 5.905725499, 7.735138864],
                   [5.717741934, 7.735138864, 5.905725499],
                   [7.859324539, 5.982999178, 7.894988604],
                   [7.859324539, 7.894988604, 5.982999178]]) / 2
    ...
    for i, S in enumerate(Ss):
        ...
        # client.sendInfo(f'eps_inc: {eps_inc}, eps_int: {eps_int}, eps_mat: {eps_mat}, t_int: {t}')
        client.sendInfo(f'eps_inc: {eps_inc}, eps_int: {eps_int}, eps_mat: {eps_mat}, t_int: {t}')
        _, remesh = get_eps_effective(eps_inc, eps_int, eps_mat, t, R, S, rf, target)
