...
summary = results + '/eps_infty_cpmd_fine.csv'
...
try:
    ...
    # fillet radius (use VdW radius for silver)
    rf = 1.72
    #rfs = np.arange(0.2, 2.81, 0.2)

    client.sendInfo(f'Beginning Calculations...')
    #for rf in rfs:
    for i, vl in enumerate(vls):
        # vol = 0.067548081 * 12.926**3    # 145.8832635203091
        vol = (vl / 100) * Ss[i]**3
        R = get_filleted_cube(vol, rf) / 2  # get sidelength of inclusion
        Ss[i] = (vol / (vl / 100))**(1/3)
        for j, t in enumerate(ts):
            # remesh only when t or vl changes
            generate_mesh(t, [R]*3, [Ss[i]]*3, rf, per2pi=20, lcoarse=1.5)
            for eps_inc in eps_incs:
                for eps_int in eps_ints:
                    # client.sendInfo(f'eps_inc: {eps_inc}, eps_int: {eps_int}, eps_mat: {eps_mat}, t_int: {t}')
                    client.sendInfo(f'eps_inc: {eps_inc}, eps_int: {eps_int}, eps_mat: {eps_mat}, t_int: {t}')
                    _, remesh = get_eps_effective(eps_inc, eps_int, eps_mat, t, [R]*3, [Ss[i]]*3, rf, target)
